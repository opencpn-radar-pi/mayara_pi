...
elseif ($ENV{APPVEYOR})
  ...
  if ("${GIT_REPOSITORY_EXISTS}" STREQUAL "0")
    ...
    # Read the "origin" remote directly instead of parsing `git status
    # --porcelain -b` for a tracking-branch indicator: a tagged release is
    # built from a detached HEAD (no tracking branch at all -- "## HEAD (no
    # branch)"), which always fell through to "not tracking", leaving
    # GIT_REPOSITORY empty and the packaged metadata's <source> a bare
    # "https://github.com/". actions/checkout always sets up "origin"
    # regardless of whether the ref is a branch or a tag, so this works in
    # both cases.
    execute_process(
