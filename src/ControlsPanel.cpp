...
void ControlsBody::Rebuild() {
  ...
  for (const char* cat : categories) {
    ...
    AddCollapsibleSection(root, ServerCategory(cat), cat,
...
void ControlsBody::AddEnum(wxSizer* outer, const ControlDef& def,
  ...
  if (as_buttons) {
    ...
    for (int v : values) {
      const wxString label = ServerEnumLabel(def, v);
...
void ControlsBody::AddButton(wxSizer* outer, const ControlDef& def) {
  auto* b = new ThemedButton(this, ServerControlName(def),
...
void ControlsBody::AddPlaceholder(wxSizer* outer, const ControlDef& def) {
  outer->Add(new wxStaticText(
                 this, wxID_ANY,
                 ServerControlName(def) + " (" + wxString::FromUTF8(def.dataType.c_str()) + ")"),
             0, wxALL, 4);
}
